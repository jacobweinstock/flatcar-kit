...
# Platforms for the multi-arch container image (`make release`).
...
clean: ## Remove build artifacts.
	rm -rf $(OUT_DIR)/$(BINARY)-* $(OUT_DIR)/linters coverage.out
...
.PHONY: release
release: $(RELEASE_BINARY_DEP) ## Build and push the multi-arch image to the registry (assumes buildx + auth).
	docker buildx build \
		--platform $(PLATFORMS) \
		--build-arg BUILD_IN_CONTAINER=$(BUILD_IN_CONTAINER) \
		-t $(IMAGE):$(VERSION) \
